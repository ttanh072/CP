...
int handValue(bool flush){
		...
		if(hand[4] == 12){
			...
			for(int i = 4; i > 0; --i){
				...
				if(hand[i] - 1 != hand[i - 1]){
					...
					*/
					else if(hand[i - 1] < 8 && hand[i] >= 8 && i != hand[i - 1] + 1) straight = false;
				}
			}
		}
	...
	// If the first card of hand is Ace.
		...
		for(int i = 0; i < 4; ++i){
			...
			if(hand[i] + 1 != hand[i + 1])	straight = false;
		}
	}
	...
	if(straight && flush){
		values.resize(5);
		for(int i = 0; i < 5; ++i){
			if(hand[i] == 0) values[i] = 13;
			else values[i] = hand[i];
		}
	...
	// If the hand is not a straight and flush.
		...
		for(int i = 0; i < 13 && !fourKind  && currentCards  < 5; ++i){
			switch(frequence[i]){
				...
				case 3:
				    if(pair) fullHouse = true;
					else third = true;
					currentCards += 3;
                    break;
				case 2:
					if(third) fullHouse = true;
					else if(pair) twoPair = true;
					else pair = true;
					currentCards += 2;
                    break;
			}
		...
		if(fourKind){
			...
			for(int i = 0; i < 5; ++i){
				if(hand[i] == 0) values[i] = 13;
				else values[i] = (!hand[i]?13:hand[i]);
			}
			sort(values.begin(), values.end());
			handValue = 5;
		}
